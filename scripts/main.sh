...
show_help() {
    ...
    echo -e "  -n, --name STACK_NAME     Nome da stack (padrão: sinapcrm)"
    ...
    echo -e "  $0 up -n sinapcrm1 -b 3000 -f 3001"
    echo -e "  $0 up --name sinapcrm2 --backend-port 4000 --frontend-port 4001"
    echo -e "  $0 up -n sinapcrm3 -b 5000 -f 5001 -c 2 -m 2048"
    echo -e "  $0 up -n sinapcrm4 -u https://api.exemplo.com -w https://app.exemplo.com"
    echo -e "  $0 up -n sinapcrm5 --color verde --tab-name 'Meu Chat'"
    echo -e "\n  # 💰 Criar instância com módulo financeiro habilitado"
    echo -e "  $0 up -n sinapcrm-finance -e -g CLIENT_ID -s CLIENT_SECRET -p PIX_KEY"
    echo -e "  $0 up --name sinapcrm-finance --enable-financial --gerencianet-client-id CLIENT_ID --gerencianet-client-secret CLIENT_SECRET --gerencianet-pix-key PIX_KEY"
    echo -e "\n  # 🔄 Atualizar instância (usa configuração salva)"
    echo -e "  $0 update -n sinapcrm1"
    echo -e "  $0 update sinapcrm1"
    echo -e "\n  # 🔄 Atualizar com novos parâmetros (atualiza configuração)"
    echo -e "  $0 update -n sinapcrm1 -c 4 -m 4096"
    echo -e "\n  # 💰 Atualizar módulo financeiro"
    echo -e "  $0 update -n sinapcrm1 -e -g NEW_CLIENT_ID -s NEW_CLIENT_SECRET -p NEW_PIX_KEY"
    echo -e "\n  # 🛠️  Gerenciar instâncias"
    echo -e "  $0 instances                    # 📋 Lista instâncias salvas"
    echo -e "  $0 down -n sinapcrm1          # 🛑 Para e remove do arquivo"
    echo -e "  $0 logs -n sinapcrm1"
    echo -e "  $0 status -n sinapcrm1"
    echo -e "  $0 restart -n sinapcrm1"
    echo -e "\n${YELLOW}🔄 Formato alternativo (compatibilidade):${NC}"
    echo -e "  $0 up sinapcrm1 3000 3001"
    echo -e "  $0 down sinapcrm1"
    echo -e "  $0 logs sinapcrm1"
    echo -e "  $0 status sinapcrm1"
    echo -e "  $0 restart sinapcrm1"
