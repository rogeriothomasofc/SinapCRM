...
const WhatsAppModal = ({ open, onClose, whatsAppId }) => {
  ...
  useEffect(() => {
    api.get("/users/list").then(({ data }) => {
      if (whatsAppId) {
        setSelectedUserIds(data.filter(u => u.whatsappId === whatsAppId).map(u => u.id));
      }
    }).catch(() => {});
  }, [whatsAppId]);
  ...
  const handleSaveWhatsApp = async (values) => {
    ...
    try {
      ...
      await api.put(`/whatsapp/${savedId}/users`, { userIds: selectedUserIds });
            ...
            <Form>
              <DialogContent dividers>
                ...
                <UserSelect
                  ...
                  title="Atendentes desta conexão"
                />
                <FormControl
                  margin="dense"
                  variant="outlined"
