...
const FlowBuilderSingleBlockModal = ({
  ...
  const handleVariableClick = (item) => {
    const text = variableFormatter(item);
    navigator.clipboard.writeText(text).then(() => {
      toast.success(`Copiado: ${text}`);
    });
  };

  const handleVariableDragStart = (e, item) => {
    e.dataTransfer.setData("text/plain", variableFormatter(item));
          ...
          >
            ...
            <Box style={{ width: "100%" }}>
              <Typography style={{ fontSize: 13, color: "#666", marginBottom: 8 }}>
                Variáveis — clique para copiar ou arraste para o campo de texto
              </Typography>
              {variables && variables.length > 0 && (
                <Box style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {variables.map((item) => (
                    <Tooltip key={item} title="Clique para copiar · Arraste para inserir" placement="top">
                      <Chip
                        label={variableFormatter(item)}
                        size="small"
                        draggable
                        onClick={() => handleVariableClick(item)}
                        onDragStart={(e) => handleVariableDragStart(e, item)}
                        style={{
                          cursor: "grab",
