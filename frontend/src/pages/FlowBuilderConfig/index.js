            ...
            {triggerPanelOpen && (
              <Box style={{
                ...
                <Box style={{
                  ...
                  <FlashOnIcon style={{ color: "white", fontSize: 16, marginRight: 6 }} />
                  <Typography style={{ flex: 1, fontWeight: 700, fontSize: 13, color: "white" }}>
                    Acionamentos
                  </Typography>
                  <IconButton size="small" onClick={() => setTriggerPanelOpen(false)} style={{ color: "rgba(255,255,255,0.8)", padding: 2 }}>
                    <CloseIcon style={{ fontSize: 16 }} />
                  </IconButton>
                </Box>
                ...
                <Box style={{ padding: "8px 10px 4px" }}>
                  ...
                  {[
                    { key: "any", label: "Qualquer mensagem recebida" },
                    { key: "firstOfDay", label: "1ª mensagem do cliente no dia" },
                    { key: "firstEver", label: "1ª mensagem do cliente" },
                  ].map(item => (
                    <Box
                      key={item.key}
                      style={{
                        ...
                        marginBottom: 4,
                        borderRadius: 6,
                        backgroundColor: triggerConfig[item.key] ? "rgba(103,58,183,0.07)" : "#FAFAFA",
                        border: `1px solid ${triggerConfig[item.key] ? "rgba(103,58,183,0.22)" : "#EFEFEF"}`,
                        cursor: "pointer",
                        transition: "all 0.15s ease",
                      }}
                      onClick={() => setTriggerConfig(p => {
                        const next = !p[item.key];
                        return { ...p, any: false, keyword: false, firstOfDay: false, firstEver: false, [item.key]: next };
                      })}
                    >
                      <Typography style={{ flex: 1, fontSize: 12, fontWeight: 500, color: triggerConfig[item.key] ? "#512DA8" : "#555" }}>
                        {item.label}
                      </Typography>
                  ...
                  <Box style={{
                    ...
                    marginBottom: 4,
                    borderRadius: 6,
                    backgroundColor: triggerConfig.keyword ? "rgba(103,58,183,0.07)" : "#FAFAFA",
                    border: `1px solid ${triggerConfig.keyword ? "rgba(103,58,183,0.22)" : "#EFEFEF"}`,
                    transition: "all 0.15s ease",
                  }}>
                    <Box style={{ display: "flex", alignItems: "center", cursor: "pointer" }}
                      onClick={() => setTriggerConfig(p => ({ ...p, keyword: !p.keyword, any: false, firstOfDay: false, firstEver: false }))}>
                      <Typography style={{ flex: 1, fontSize: 12, fontWeight: 500, color: triggerConfig.keyword ? "#512DA8" : "#555" }}>
                    ...
                    {triggerConfig.keyword && (
                      <Box style={{ marginTop: 8 }}>
                        <Box style={{
                          padding: "6px 8px", backgroundColor: "white", borderRadius: 5,
                          border: "1px solid #EDE7F6", marginBottom: 6,
                          minHeight: 30, display: "flex", flexWrap: "wrap", gap: 3,
                        }}>
                          {triggerConfig.keywords.length === 0
                            ? <Typography style={{ fontSize: 11, color: "#BDBDBD", lineHeight: "20px" }}>Nenhuma palavra</Typography>
                            : triggerConfig.keywords.map((kw, i) => (
                              <Chip key={i} label={kw} size="small"
                                style={{ backgroundColor: "#673AB7", color: "white", fontSize: 11, height: 22 }}
                                onDelete={() => setTriggerConfig(p => ({ ...p, keywords: p.keywords.filter((_, idx) => idx !== i) }))} />
                            ))
                          }
                        </Box>
                          ...
                          <TextField size="small" variant="outlined" fullWidth
                            ...
                            onKeyPress={e => {
                              if (e.key === "Enter" && keywordInput.trim()) {
                                setTriggerConfig(p => ({ ...p, keywords: [...p.keywords, keywordInput.trim()] }));
                                setKeywordInput("");
                              }
                            }}
                          />
                          <Button variant="contained" size="small"
                            style={{ backgroundColor: "#673AB7", color: "white", minWidth: 70, fontSize: 11 }}
                            onClick={() => {
                ...
                <Box style={{ padding: "8px 10px 10px", borderTop: "1px solid #F0F0F0" }}>
                  <Typography style={{ fontSize: 10, fontWeight: 700, color: "#BDBDBD", letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 6 }}>
                    Intervalo de reativação
                  </Typography>
                  <Box style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <TextField size="small" variant="outlined" type="number"
                      value={triggerConfig.reactivationInterval}
                      onChange={e => setTriggerConfig(p => ({ ...p, reactivationInterval: parseInt(e.target.value) || 0 }))}
                      style={{ width: 72 }} inputProps={{ min: 0 }}
                    />
                    <Select variant="outlined" value={triggerConfig.reactivationUnit}
                      onChange={e => setTriggerConfig(p => ({ ...p, reactivationUnit: e.target.value }))}
            ...
            {rulesPanelOpen && (
              <Box style={{
                ...
                <Box style={{
                  ...
                  <RuleIcon style={{ color: "white", fontSize: 16, marginRight: 6 }} />
                  <Typography style={{ flex: 1, fontWeight: 700, fontSize: 13, color: "white" }}>
                    Regras
                  </Typography>
                  <IconButton size="small" onClick={() => setRulesPanelOpen(false)} style={{ color: "rgba(255,255,255,0.8)", padding: 2 }}>
                    <CloseIcon style={{ fontSize: 16 }} />
                  </IconButton>
                </Box>
                ...
                <Box style={{ padding: "8px 10px 4px" }}>
                  ...
                  {[
                    { key: "allowGroups",         label: "Permitir responder a grupos" },
                    { key: "onlyScheduledHours",  label: "Permitir respostas apenas em dias e horários determinados" },
                    { key: "noResponseIfOpen",    label: "Não responder se a conversa estiver aberta" },
                    { key: "customSignature",      label: "Personalizar ou desativar a assinatura" },
                    { key: "simulateTyping",       label: "Simular que está digitando ou gravando áudio" },
                  ].map(item => (
                    <Box
                      key={item.key}
                      style={{
                        ...
                        marginBottom: 4,
                        borderRadius: 6,
                        backgroundColor: rulesConfig[item.key] ? "rgba(103,58,183,0.07)" : "#FAFAFA",
                        border: `1px solid ${rulesConfig[item.key] ? "rgba(103,58,183,0.22)" : "#EFEFEF"}`,
                        cursor: "pointer",
                        transition: "all 0.15s ease",
                      }}
                      onClick={() => setRulesConfig(p => ({ ...p, [item.key]: !p[item.key] }))}
                    >
                      <Typography style={{ flex: 1, fontSize: 12, fontWeight: 500, color: rulesConfig[item.key] ? "#512DA8" : "#555", lineHeight: 1.35 }}>
                        {item.label}
                      </Typography>
                ...
                {/* Regras de CRM */}
                <Box style={{ padding: "8px 10px 4px", borderTop: "1px solid #F0F0F0" }}>
                  <Typography style={{ fontSize: 10, fontWeight: 700, color: "#BDBDBD", letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 6 }}>
                    Regras de CRM
                  </Typography>
                  {[
                    { key: "noCrmContacts", label: "Não responder se o contato estiver em um CRM", exclusive: false },
                    { key: "noCrm",         label: "Não responder os CRMs", exclusive: true },
                    { key: "onlyCrm",       label: "Responder apenas os CRMs", exclusive: true },
                  ].map(item => (
                    <Box
                      key={item.key}
                      style={{
                      ...
                      onClick={() => setRulesConfig(p => {
                        if (item.exclusive) {
                          return { ...p, noCrm: false, onlyCrm: false, [item.key]: !p[item.key] };
                        }
                        return { ...p, [item.key]: !p[item.key] };
                      })}
                    >
                      <Typography style={{ flex: 1, fontSize: 12, fontWeight: 500, color: rulesConfig[item.key] ? "#512DA8" : "#555", lineHeight: 1.35 }}>
                        {item.label}
                ...
                <Box style={{ padding: "8px 10px 10px", borderTop: "1px solid #F0F0F0" }}>
                  ...
                  {[
                    { key: "noTaggedContacts", label: "Não responder se o contato estiver etiquetado", exclusive: false },
                    { key: "noTags",           label: "Não responder as etiquetas", exclusive: true },
                    { key: "onlyTags",         label: "Responder apenas as etiquetas", exclusive: true },
                  ].map(item => (
                    <Box
                      key={item.key}
                      style={{
                      ...
                      onClick={() => setRulesConfig(p => {
                        if (item.exclusive) {
                          return { ...p, noTags: false, onlyTags: false, [item.key]: !p[item.key] };
                        }
                        return { ...p, [item.key]: !p[item.key] };
                      })}
                    >
