...
export const FlowImport = async (req: Request, res: Response): Promise<Response> => {
  const { companyId } = req.user;
  const userId = parseInt(req.user.id);
  const { name, nodes, edges, connections, variables } = req.body;

  if (!name) {
    return res.status(400).json({ error: "Nome do agente é obrigatório" });
  }

  const flowConnections = connections || edges || [];
  const flowNodes = nodes || [];

  const existing = await FlowBuilderModel.findOne({
    where: { name, company_id: companyId }
  });
  const flowName = existing ? `${name} (importado)` : name;

  const flow = await FlowBuilderModel.create({
    user_id: userId,
    company_id: companyId,
    name: flowName,
    flow: { nodes: flowNodes, connections: flowConnections },
    variables: variables || []
  });
...
export const FlowToggleActive = async (req: Request, res: Response): Promise<Response> => {
  const { companyId } = req.user;
  const { flowId, active } = req.body;
  await FlowBuilderModel.update({ active }, { where: { id: flowId, company_id: companyId } });
